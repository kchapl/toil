...
object DateHelper {
  ...
  def lastDayOfNextMonth(date: LocalDate): LocalDate = firstDayOfNextMonth(date).plusMonths(1).minusDays(1)

  private def yearMonthOfNextSeason(date: LocalDate): YearMonth = {
    val month = date.getMonthValue / 3 * 3 + 3
    if (month > 12) YearMonth.of(date.getYear + 1, month - 12) else YearMonth.of(date.getYear, month)
  }
